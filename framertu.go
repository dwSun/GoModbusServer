...
func NewRTUFrame(packet []byte) (*RTUFrame, error) {
	// Check the that the packet length is sufficient

	packetLen := len(packet)

	if packetLen < min_ADU_RTU {
		return nil, fmt.Errorf("RTU Frame error: packet less than 5 bytes: %v", packet)
	}

	// Check the CRC.
	crcGot := binary.LittleEndian.Uint16(packet[packetLen-2 : packetLen])
	crcCalc := crcModbus(packet[0 : packetLen-2])
	if crcCalc != crcGot {
		return nil, fmt.Errorf("RTU Frame error: CRC (expected 0x%x, got 0x%x)", crcCalc, crcGot)
	}
